...
class ContrastiveLoss(nn.Module):
    """用于学习特征表示，使得同类样本的特征向量在特征空间中更接近，不同类样本的特征向量更远离
...
class CircleLoss(nn.Module):
    """改进的Softmax损失, 为每个相似度得分分配不同的权重, 让优化更加灵活
...
class Arcface(nn.Module):
    r"""在角度空间中增加间隔，使得类内更紧凑，类间更分离, 将Softmax损失从余弦空间转换到角度空间
        通过增加角度间隔 m 来增强判别性
...
class Cosface(nn.Module):
    r"""在余弦空间中直接增加间隔
        相比于Arcface, Cosface是在余弦值上直接减去一个固定值
...
class AMSoftmax(nn.Module):
    """Cosface的另一种实现形式, 与Cosface原理相同但实现方式略有不同
